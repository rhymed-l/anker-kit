...
/anker.specify Real-time chat system with message history and user presence
...
/anker.plan WebSocket for real-time messaging, PostgreSQL for history, Redis for presence
...
2. **Don't guess**: If the prompt doesn't anker-spa something, mark it
```
...
```text
Every feature in AnkerSPA MUST begin its existence as a standalone library.
